...
import com.zhao.temporal.Utils.HTMLParser;
...
public class TargetPage extends Page{
	...
	TargetPage(String absPath_Page, String urlString, String timestamp) throws NoSuchAlgorithmException, IOException {
		File file_historicalPage = new File(absPath_Page);
		pageInit(file_historicalPage, urlString, timestamp, 50, 0.7);
	}
	
	TargetPage(File file_Page, String urlString, String timestamp) throws NoSuchAlgorithmException, IOException {
	...
	private void pageInit(File file_Page, String urlString, String timestamp, int lenThreshold, double simThreshold) throws NoSuchAlgorithmException, IOException {
		...
		List<String> ps_FromHTML = HTMLParser.getPfromHTML(file_Page, lenThreshold);
		...
		for (String p_Filtered: ps_Filtered) {
			...
			int startPos = FileProcess.findPosFromHTML(p_Filtered, file_Page);
			if (startPos != -1) {
	...
	public void toFile(File file_TaggedTargetPage) {
		...
		//	LENGTH THRESHOLD
		FileProcess.addLinetoaFile("#Length Threshold: " + "50", filePath_TaggedTargetPage);
		
		//	SIMILARITY THRESHOLD
		FileProcess.addLinetoaFile("#Similarity Threshold: " + "JaroWinklerTFIDF " + "0.7", filePath_TaggedTargetPage);
